...
- CC FLOW is a macOS menu bar app that surfaces Dynamic Island-style status for Claude Code, Codex, and four compatible TRAE variants.
- Claude Code and Codex are the default integrations. TRAE, TRAE CN, TRAE WORK, and TRAE WORK CN are optional compatibility clients shown when detected or already configured.
- The main runtime path is:
  - Claude Code, Codex, or TRAE hook events
...
- Flow Island left region (compact feature / expanded feature container / session detail): `CCFlow/UI/Views/FlowIslandLeftRegion.swift` (expanded slot renders `LeftFeatureContainerView`), `CCFlow/UI/Views/NotchView.swift` headerRow (compact slot)
  ...
  - Expanded panel size is draggable from the bottom-left/bottom-right corners of the opened Flow Island (subtle rounded corner indicators overlaid in `NotchView`); live size goes through `NotchViewModel.openedSizeOverride`, and the final size is persisted per-feature in `LeftFeature.expandedWidth` / `expandedHeight` (fallback to the usage-panel baseline, 680×460pt)
