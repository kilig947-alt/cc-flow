...
actor DiagnosticsExporter {
    ...
    func exportArchive(to destinationURL: URL) async throws -> DiagnosticsExportResult {
        ...
        let tempRoot = fileManager.temporaryDirectory
            .appendingPathComponent("CCFlow-Diagnostics-\(UUID().uuidString)", isDirectory: true)
        let exportRoot = tempRoot.appendingPathComponent("CCFlow-Diagnostics-\(timestamp)", isDirectory: true)
        var warnings: [String] = []
    ...
    private func writeRedactedTailCopy(from sourceURL: URL, to destinationURL: URL) throws {
        ...
        var output = result.wasTruncated
            ? "[CC FLOW diagnostics: file truncated to last \(Self.maxDebugFileBytes) bytes]\n"
            : ""
    ...
    private func writeUnifiedLogs(to destinationURL: URL) async throws {
        let predicate = "subsystem == \"ai.ccflow.app\""
        try await writeCommandOutput(
    ...
    private func preferredTraeHookDebugDirectory() -> URL {
        BridgeRuntimePaths.logsDirectoryURL.appendingPathComponent("claude-hooks", isDirectory: true)
    }
