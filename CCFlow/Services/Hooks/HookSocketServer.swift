...
private let logger = Logger(subsystem: "ai.ccflow.app", category: "Hooks")

extension Notification.Name {
    /// Posted only after a valid Codex bridge envelope reaches the app's hook ingress.
    nonisolated static let ccFlowCodexHookEventReceived = Notification.Name("ccFlowCodexHookEventReceived")
}
...
private extension BridgeEnvelope {
    ...
    var resolvedSessionID: String {
        let rawSessionID = metadata["session_id"]
            ?? metadata["thread_id"]
            ?? metadata["threadId"]
            ?? sessionKey.components(separatedBy: ":").dropFirst().joined(separator: ":")
        guard !rawSessionID.isEmpty else { return sessionKey }
        let prefix = provider.rawValue + ":"
        return rawSessionID.hasPrefix(prefix) ? rawSessionID : prefix + rawSessionID
    }
    ...
    private static func makeClientInfo(
        ...
        if let matchedProfile {
            ...
            kind = provider.sessionProvider.defaultClientKind
        }
        ...
        let terminalBundleVariant = provider == .trae
            ? terminalBundleID.flatMap { TraeVariant.fromBundleIdentifier($0) }
            : nil
        let namedTraeVariant = provider == .trae
            ? TraeVariant.fromClientIdentity(effectiveExplicitName, explicitMetadataOriginator)
            : nil
        let resolvedBundleID = explicitBundleID
            ?? terminalBundleVariant?.bundleIdentifier
            ?? namedTraeVariant?.bundleIdentifier
        ...
        if let effectiveExplicitName {
            ...
            resolvedName = resolvedProfile?.displayName
                ?? provider.sessionProvider.displayName
        }
...
private extension SessionProvider {
    var defaultClientKind: SessionClientKind {
        switch self {
...
class HookSocketServer {
    ...
    static let healthCheckRequests: Set<String> = [
        #"{"type":"cc-flow-health-check"}"#,
        #"{"type":"trae-flow-health-check"}"#
    ]
    static let healthCheckResponse = #"{"ok":true}"#

    static func decodeBridgeEventForTesting(_ data: Data) throws -> HookEvent {
    ...
    private let queue = DispatchQueue(label: "ai.ccflow.app.socket", qos: .userInitiated)
    ...
    private func handleClient(_ clientSocket: Int32) {
        ...
        if let request = String(data: allData, encoding: .utf8)?
            .trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        if envelope.provider == .codex {
            NotificationCenter.default.post(name: .ccFlowCodexHookEventReceived, object: nil)
        }
