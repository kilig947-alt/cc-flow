...
PACKAGE_PATH="${CC_FLOW_BRIDGE_PACKAGE_PATH:-${TRAE_FLOW_BRIDGE_PACKAGE_PATH:-$SRCROOT/Prototype}}"
PRODUCT_NAME="${CC_FLOW_BRIDGE_PRODUCT_NAME:-${TRAE_FLOW_BRIDGE_PRODUCT_NAME:-CCFlowBridge}}"
BUILD_CONFIGURATION=$(echo "${CONFIGURATION:-Debug}" | tr '[:upper:]' '[:lower:]')
SCRATCH_BASE="${DERIVED_FILE_DIR:?DERIVED_FILE_DIR is required}/CCFlowBridge-build"
